resource "aws_security_group" "singleton_ec2_secgroup" {
  name = var.server_number != "" ? "${var.instance_name_tag_prefix}-${var.subnet_name}-${var.server_number}" : "${var.instance_name_tag_prefix}-${var.subnet_name}"
...
resource "aws_instance" "singleton_ec2_instance" {
  ...
  tags = {
    ...
    backup = "true"
    monitor_level = var.monitor_level != "" ? "${var.monitor_level}" : "None"
  }
